...
ghb_spd[0] = ghb_period
 ...
 # build the observation data in a dictionary. The dictionary key is the filename of the observation output file and optionally
 # a “binary” keyword to make the file binary. When the optional “binary” keyword is used the dictionary key is a tuple, otherwise
 # it is a string. The dictionary value is a list of tuples containing the contents of the observation package’s continuous block,
 # with each tuple containing one line of information.
# build obs data
ghb_obs = {
    ("ghb_obs.csv", "binary"): [
...
# The observations can then be added to the ghb package using the obs attribute’s initialize method. 
# The observation package’s file name, digits, and print_input options, along with the continuous 
# block data are set in the initialize method.
